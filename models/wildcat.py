...
class WildcatPool2dFunction(Function):
    ...
    def forward(self, input):
        ...
        if kmin > 0 and self.alpha is not 0:
            ...
            output.add_(sorted.narrow(
                2, n - kmin, kmin).sum(2).mul_(self.alpha / kmin)).div_(2)
    ...
    def backward(self, grad_output):
        ...
        grad_output_max = grad_output.view(
            batch_size, num_channels, 1).expand(batch_size, num_channels, kmax)
        ...
        if kmin > 0 and self.alpha is not 0:
            grad_output_min = grad_output.view(
                batch_size, num_channels, 1).expand(batch_size, num_channels, kmin)
...
def resnet50_wildcat(num_classes, model_path_prefix=None, kmax=1, kmin=None, alpha=1, num_maps=1):
    model = models.resnet50(False)
    model.load_state_dict(torch.load(os.path.join(
        model_path_prefix, 'resnet50-19c8e357.pth')))
...
def resnet101_wildcat(num_classes, model_path_prefix=None, kmax=1, kmin=None, alpha=1, num_maps=1):
    model = models.resnet101(False)
    model.load_state_dict(torch.load(os.path.join(
        model_path_prefix, 'resnet101-5d3b4d8f.pth')))
    pooling = nn.Sequential()
    pooling.add_module('class_wise', ClassWisePool(num_maps))
    pooling.add_module('spatial', WildcatPool2d(kmax, kmin, alpha))
    return ResNetWSL(model, num_classes * num_maps, pooling=pooling)

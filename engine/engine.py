...
class Engine(object):
    ...
    def on_end_epoch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        if display:
            if training:
                ...
                logging.info('Test: \t Loss {loss:.4f}'.format(loss=loss))
        return loss
    ...
    def init_learning(self, model, criterion=None):
        ...
        if self._state('train_transform') is None:
            ...
            self.state['train_transform'] = transforms.Compose([
                ...
                normalize,
                RandomErasing(
                    probability=self.state['re_probability'],
            ...
            logging.info(self.state['train_transform'])
    ...
    def learning(self, model, criterion, train_dataset, val_dataset, optimizer=None):
        ...
        if self._state('resume') is not None:
            if os.path.isfile(self.state['resume']):
                logging.info("=> loading checkpoint '{}'".format(self.state['resume']))
                ...
                logging.info("=> no checkpoint found at '{}'".format(self.state['resume']))
        ...
        for epoch in range(self.state['start_epoch'], self.state['max_epochs']):
            ...
            for param_group in optimizer.param_groups:
                logging.info(f"Last LR set as {param_group['lr']:.2e}")
                break
            ...
            logging.info(' *** best={best:.3f}'.format(best=self.state['best_score']))
    ...
    def save_checkpoint(self, state, is_best, filename='checkpoint.pth.tar'):
        if self._state('save_model_path') is not None:
            filename_ = filename
            # filename_ = f'checkpoint{self.state["epoch"]}.pth.tar'

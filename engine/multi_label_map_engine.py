...
class MultiLabelMAPEngine(Engine):
    ...
    def on_end_epoch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        if display:
            if training:
                logging.info(f'Epoch: [{self.state["epoch"]}]\t'
                             f'Loss {loss:.4f}\t'
                             f'mAP {mAP:.3f}')
                logging.info(f'OP: {OP:.4f}\t'
                             f'OR: {OR:.4f}\t'
                             f'OF1: {OF1:.4f}\t'
                             f'CP: {CP:.4f}\t'
                             f'CR: {CR:.4f}\t'
                ...
                logging.info(f'Test: \t Loss {loss:.4f}\t mAP {mAP:.3f}')
                logging.info(f'OP: {OP:.4f}\t'
                             f'OR: {OR:.4f}\t'
                             f'OF1: {OF1:.4f}\t'
                             f'CP: {CP:.4f}\t'
    ...
    def on_start_batch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        if optimizer is not None:
            current_iter = self.state['iteration'] + \
                self.state['epoch'] * len(data_loader)

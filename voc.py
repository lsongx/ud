...
class Voc2007Classification(data.Dataset):
    def __init__(self, root, set, transform=None, target_transform=None, inp_name=None, adj=None, only_hard=False):
        self.root = root
        ...
        self.only_hard = only_hard
        if self.only_hard:
            self.idx_mapping = {}
            idx = 0
            for j, (_,label) in enumerate(self.images):
                if (label==0).sum() > 0:
                    self.idx_mapping[idx] = j
                    idx += 1
